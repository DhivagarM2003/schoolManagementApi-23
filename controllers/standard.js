...
class Standard {
    ...
    static async deleteStandard(db, standard_name) {
        try {
            ...
            db.Sequelize.query(`DROP TABLE marks${standard_name};`);
            db.Sequelize.query(`DROP TABLE attendance${standard_name};`);
    ...
    static async viewstandard() {
        try {
            const standards = await db.Sequelize.query(`SELECT * FROM standards ;`);
            return standards[0];
        } catch (error) {
            console.error('Error deleting Standard:', error);
            throw error;
